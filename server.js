...
const REQUIRED_ENV = ["SMTP_USER", "SMTP_APP_PASSWORD", "COLLAB_EMAIL"];
...
function makeTransporter() {
  if (!process.env.SMTP_USER || !process.env.SMTP_APP_PASSWORD) {
    throw new Error("SMTP credentials missing. Set SMTP_USER and SMTP_APP_PASSWORD in your .env file.");
  }
...
// ── Internal email (to Collab) ──
function internalHtml(data) {
  const now = new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" });
  const row = (label, val) =>
    `<tr><td style="padding:8px 16px 8px 0;color:#888;vertical-align:top;width:110px;font-size:13px;">${label}</td><td style="padding:8px 0;color:#1a1a1a;font-size:13px;">${esc(val || "Not provided")}</td></tr>`;
...
function customerHtml(data) {
  ...
  const summaryRow = (label, val) => val
    ? `<tr><td style="padding:5px 16px 5px 0;color:#888;font-size:13px;width:80px;">${label}</td><td style="padding:5px 0;color:#1a1a1a;font-size:13px;">${esc(val)}</td></tr>`
    : "";
...
<body style="margin:0;padding:24px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f4f4f0;color:#1a1a1a;">
  <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:6px;padding:36px;border:1px solid #e0e0d8;">
    ...
    <p style="margin:0 0 20px;font-size:14px;line-height:1.7;color:#1a1a1a;">We've received your project enquiry successfully. We'll review the details you sent and get back to you shortly to discuss the project and the next steps.</p>

    <hr style="border:none;border-top:1px solid #e0e0d8;margin:0 0 20px;">
    <p style="margin:0 0 12px;font-size:11px;text-transform:uppercase;letter-spacing:0.1em;color:#aaa;">Your enquiry</p>
    <table style="width:100%;border-collapse:collapse;">
    ...
    <hr style="border:none;border-top:1px solid #e0e0d8;margin:20px 0;">
    <p style="margin:0 0 14px;font-size:13px;line-height:1.7;color:#555;">You don't need to submit the form again — we've got it. If you need to add anything else, you can reply directly to this email.</p>
...
We've received your project enquiry successfully. We'll review the details you sent and get back to you shortly.
...
You don't need to submit the form again — we've got it. If you need to add anything else, you can reply directly to this email.
...
app.post("/api/enquiry", async (req, res) => {
  try {
    ...
    await Promise.all([
      // Email 1: Internal notification to Collab
      transporter.sendMail({
        from:    `"Collab Enquiries" <${senderEmail}>`,
        to:      collabEmail,
        replyTo: clean.email,
    ...
    // Log details server-side but never expose to client
    if (err.message?.includes("SMTP") || err.code === "EAUTH") {
      console.error("[server] SMTP auth error — check SMTP_USER and SMTP_APP_PASSWORD in .env:", err.message);
    } else {
      console.error("[server] Enquiry error:", err);
    }
    return res.status(500).json({ success: false, message: "We couldn't send your enquiry right now. Please try again or email us directly at collabstudio.in@gmail.com" });
  }
});
